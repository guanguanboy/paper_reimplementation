...
class LPTN(nn.Module):
    ...
    def forward(self, real_A_full):
        ...
        fake_B_low = self.trans_low(pyr_A[-1]) #使用金字塔的最后一层来恢复亮度，颜色等信息
        ...
        print("Pyr_A:")
        print(high_with_low.shape, len(pyr_A), fake_B_low.shape)
        for i in range(len(pyr_A)):
            print(pyr_A[i].shape)

        pyr_A_trans = self.trans_high(high_with_low, pyr_A, fake_B_low)
        print("pyr_A_trans:")
        for i in range(len(pyr_A_trans)):
            print(pyr_A_trans[i].shape)

...
class SwinIR(nn.Module):
    ...
    def __init__(self, k=36, img_size=64, patch_size=1, in_chans=3,out_chans=3,
        ...
        self.conv_first = ShallowFeatureExtractor(k)
    ...
    def forward(self, x, x_spectral):
        #self.mean = self.mean.type_as(x)
        #x = (x - self.mean) * self.img_range

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x, x_spectral)
            ...
            # for image denoising and JPEG compression artifact reduction
            x_first = self.conv_first(x, x_spectral)
...
if __name__ == '__main__':
    ...
    height = 20
    width = 20
    model = SwinIR(upscale=1, img_size=(height, width),in_chans=1, out_chans=1,
    ...
    x = model(intput_tensor_L, spectal_tensor)
    print(x.shape)

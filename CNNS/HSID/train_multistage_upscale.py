...
from model_multistage_upsample import MultiStageHSIDUpscale
...
def train_model_multistage_lowlight():
    ...
    for epoch in range(start_epoch, num_epoch+1):
        ...
        for batch_idx, (noisy, cubic, label) in enumerate(train_loader):
            ...
            label_half_scale = F.interpolate(label, scale_factor=0.5, mode="bilinear")
            label_quarter_scale = F.interpolate(label, scale_factor=0.25, mode="bilinear")

            loss = loss_fuction(residual[0], label)/(label.shape[1]*label.shape[2]*label.shape[3]) \
             + loss_fuction(residual[1], label_half_scale)/(label_half_scale.shape[1]*label_half_scale.shape[2]*label_half_scale.shape[3]) \
                  + loss_fuction(residual[2], label_quarter_scale)/(label_quarter_scale.shape[1]*label_quarter_scale.shape[2]*label_quarter_scale.shape[3])
        ...
        }, f"checkpoints/hsid_multistage_upscale_patchsize64_{epoch}.pth")
        ...
        if psnr > best_psnr:
            ...
            torch.save({
                ...
                'gen': net.state_dict(),
                'gen_opt': hsid_optimizer.state_dict(),
            }, f"checkpoints/hsid_multistage_upscale_patchsize64_best.pth")

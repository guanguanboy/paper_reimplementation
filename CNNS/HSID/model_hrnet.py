...
class BasicBlock(nn.Module):
    expansion = 1 #basicBlockx中不对通道进行扩展expansion
...
class Bottleneck(nn.Module): #通道数先降后升，这应该就是叫bottleneck的原因
    expansion = 4 #将通道数扩展四倍
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        self.branches = self._make_branches( #创建branch加深层数
            num_branches, blocks, num_blocks, num_channels)
        self.fuse_layers = self._make_fuse_layers() #创建特征融合层
...
class HyperHRNet(nn.Module):
    def __init__(self, k=24):
        self.inplanes = 64   #而self.inplane表示当前特征图通道数

        super(HyperHRNet, self).__init__()
        ...
        self.relu = nn.ReLU()
        self.conv1 = nn.Sequential(*conv_relu(120, 64))

        self.layer1 = self._make_layer(Bottleneck, 64, 4)

        self.stage2_cfg = self._get_stage2_cfg()
        ...
        self.stage3_cfg = self._get_stage3_cfg()
        ...
        self.stage4_cfg = self._get_stage4_cfg()
    ...
    def _get_stage2_cfg(self):
        stage2_cfg = {'NUM_CHANNELS' : [32, 64],
                        'NUM_BLOCKS' : [4, 4],
                        'NUM_MODULES' : 1,
                        'NUM_BRANCHES' : 2,
                        'FUSE_METHOD' : 'SUM',
    ...
    def _get_stage3_cfg(self):
        stage3_cfg = {'NUM_CHANNELS' : [32, 64, 128],
                        'NUM_BLOCKS' : [4, 4, 4],
                        'NUM_MODULES' : 4,
                        'NUM_BRANCHES' : 3,
                        'FUSE_METHOD' : 'SUM',
    ...
    def _get_stage4_cfg(self):
        stage4_cfg = {'NUM_CHANNELS' : [32, 64, 128, 256],
                        'NUM_BLOCKS': [4, 4, 4, 4],
                        'NUM_MODULES' : 3,
                        'NUM_BRANCHES' : 4,
                        'FUSE_METHOD' : 'SUM',
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion: #当stride不等于1的时候，或者通道数不想等的时候，需要对通道数进行调整
            downsample = nn.Sequential(
                nn.Conv2d(
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        num_modules = layer_config['NUM_MODULES'] #指的是HighResolutionModule
    ...
    def forward(self, x_spatial, x_spectral):
        ...
        x1 = self.conv1(feature_all) ## feature_all 是120个通道，x1是64个通道
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        y_list = self.stage4(x_list) #y_list[0]的shape是[1, 32, 64, 48]

        x = self.final_layer(y_list[0]) #y_list[0]的shape是[1, 32, 64, 48]
        return x
...
def testhrnet():
    ...
    data = torch.randn(1, 24, 64, 64)
    data1 = torch.randn(1, 1, 64, 64)

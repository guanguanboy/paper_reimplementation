...
from model_refactored import HSIDRefactored
...
def train_model_multistage_lowlight():
    ...
    for epoch in range(start_epoch, num_epoch+1):
        ...
        for batch_idx, (noisy, cubic, label) in enumerate(train_loader):
            ...
            cubic_half_scale = F.interpolate(cubic, scale_factor=0.5, mode="bilinear")
            residual_half_scale = net(noisy_half_scale, cubic_half_scale)

            loss1 = loss_fuction(residual, label-noisy)
            loss2 = loss_fuction(residual_half_scale, label_half_scale-noisy_half_scale)
            loss = loss1 + loss2

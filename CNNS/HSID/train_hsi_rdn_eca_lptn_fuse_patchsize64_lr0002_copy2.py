...
import model_utils
import dir_utils
from hsi_lptn_model import HSIRDNECA_LPTN,HSIRDNECA_LPTN_FUSE,HSIRDNECA_LPTN_FUSE_CONV,HSIRDNECA_LPTN_FUSE_CONV_Without_High,HSIRDNECA_LPTN_FUSE_CONV_Without_High_MSFE_ECA,HSIRDNECA_LPTN_FUSE_CONV_Ablation3
...
def train_model_residual_lowlight_rdn():
    ...
    save_model_path = './checkpoints/hsie_ablation/conv_num_3'
    if not os.path.exists(save_model_path):
        os.mkdir(save_model_path)

    #创建模型
    net = HSIRDNECA_LPTN_FUSE_CONV_Ablation3(k=K)
    init_params(net)
    device_ids = [0, 1]

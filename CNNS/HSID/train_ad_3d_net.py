...
from model_3d_adnet import Model_3D_ADNet
#设置超参数
NUM_EPOCHS =120
BATCH_SIZE = 32
...
def train_model_residual_lowlight():
    ...
    mat_src_path = './data/lowlight_origin_outdoor_standard/test_resized/car_label_200.mat'
    test_label_hsi = scio.loadmat(mat_src_path)['label_normalized_hsi']

    #加载测试数据
    batch_size = 1
    #test_data_dir = './data/test_lowlight/si200/'
    test_data_dir = './data/test_lowlight_outdoor/car_200/'
    ...
    save_model_path = './checkpoints/model_3d_adnet_outdoor'
    if not os.path.exists(save_model_path):
        os.mkdir(save_model_path)

    #创建模型
    net = Model_3D_ADNet(K)
    init_params(net)
    #net = nn.DataParallel(net).to(device)
    net = net.to(device)

    #创建优化器
    hsid_optimizer = optim.Adam(net.parameters(), lr=INIT_LEARNING_RATE,betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.ExponentialLR(hsid_optimizer, gamma=0.9) 
    #定义loss 函数

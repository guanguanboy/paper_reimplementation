...
def encoder_test():
    ...
    encoder = Encoder(60, 3, 4, act, False, 48, csff=False)
    decoder = Decoder(60, 3, 4, act, False, 48)

    tensor = torch.rand((128, 60, 20, 20))

    output = encoder(tensor)
    print(output[0].shape, output[1].shape, output[2].shape)

    decoder_output = decoder(output)
    print(decoder_output[0].shape, decoder_output[1].shape, decoder_output[2].shape)
...
class AttentionUNet(nn.Module):
    def __init__(self, n_feat=60, kernel_size=3,
                 reduction=4, scale_unetfeats=48, bias=False):
...
def attentionUnet_test():
    tensor = torch.rand((128, 60, 20, 20))
    unet = AttentionUNet(60, 3, 4, 48, False)

    res = unet(tensor)
    print(res.shape) #torch.Size([128, 60, 20, 20])

...
import model_utils
import dir_utils
from hsi_lptn_model import HSIRDNECA_LPTN
#设置超参数
NUM_EPOCHS =100
BATCH_SIZE = 128
...
def train_model_residual_lowlight_rdn():
    ...
    print(device)
    #准备数据
    train_set = HsiCubicTrainDataset('./data/train_lowli_outdoor_standard_patchsize32_k12/')
    ...
    #加载测试label数据
    mat_src_path = './data/lowlight_origin_outdoor_standard/test/15ms/007_2_2021-01-20_018.mat'
    test_label_hsi = scio.loadmat(mat_src_path)['label_normalized_hsi']

    #加载测试数据
    batch_size = 1
    #test_data_dir = './data/test_lowlight/cuk12/'
    test_data_dir = './data/test_lowli_outdoor_k12/007_2_2021-01-20_018/'
    ...
    save_model_path = './checkpoints/lhsie_outdoor_lptn'
    if not os.path.exists(save_model_path):
        os.mkdir(save_model_path)

    #创建模型
    net = HSIRDNECA_LPTN(K)
    init_params(net)
    device_ids = [0, 1]

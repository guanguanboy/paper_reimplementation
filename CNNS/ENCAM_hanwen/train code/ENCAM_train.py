...
BATCH_SIZE=108
#train datas
train1=np.load('cave8.npy') #(3000, 1000, 31)
train2=np.load('train_pavia8.npy') #(1096, 480, 93)
train3=np.load('train_washington8.npy') #(1080, 307, 191)


train1=train1.transpose((2,1,0)) #(31, 1000, 3000)
train2=train2.transpose((2,1,0)) #(93, 480, 1096)
train3=train3.transpose((2,1,0)) #(191, 307, 1080)


#Verification datas
test=np.load('1.npy') #(1380, 1300, 31)
test=test[:400,:400,:] #(400, 400, 31)
test=test.transpose((2,1,0)) #(31, 400, 400)
root='./'
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH):#
        ...
        for tex in range(1):
            ...
            net.train()
            train1=data_aug(train1,mode) #如果mode是1或者3，增广之后train1的shape会变化为(31, 3000, 1000)
            train2 = data_aug(train2, mode)#如果mode是1或者3，增广之后train2的shape会变化为(93, 1096, 480)
            train3=data_aug(train3,mode)#如果mode是1或者3，增广之后train3的shape会变化为(191, 1080, 307)
            ...
            data_patches = torch.from_numpy(data_patches.transpose((0, 3, 1, 2, )))#在datagenerator中增加了第3维，这里将第3维之后到第1维
            data_cubic_patches = torch.from_numpy(data_cubic_patches.transpose((0, 4, 1, 2, 3))) #在datagenerator中增加了第4维，这里将第4维置换到第1维
        ...
        data_patches = torch.from_numpy(data_patches.transpose((0, 3, 1, 2,)))#将第3维置换到第1维
        data_cubic_patches = torch.from_numpy(data_cubic_patches.transpose((0, 4, 1, 2, 3)))#将第4维置换到第1维

...
import py.model.*
addpath(genpath('lib'));
dataname = 'test_washington.mat';%  Please make sure this MSI is of size height x width x nbands and in range [0, 1].

dataRoad = ['D:\\Codes\\codes_of_paper_reimplementation\\CNNS\\ENCAM_hanwen\\test code\\' dataname];
...
% washington=washington(1:300,1:300,:);
msi_sz  =  size(washington);
[w,h, b] = size(washington);
% [60,30,20]
...
for i=1:b
%     
    noisy_msi(:, :, i)=washington(:, :, i)+randn(size(washington(:,:,i)))*sigma;
%    
end
% for i=1:b
% %     randi([15,25],1)/255.0
%     noisy_msi(:, :, i)=washington(:, :, i);
...
% for i=1:b
%     if mod(i,2)==0
%         noisy_msi(:, :, i) = washington(:, :, i) +randi([20,30],1)/255.0*randn(size(washington(:, :, i)));
%     
%     else
%         noisy_msi(:, :, i) = washington(:, :, i);
%     end
...
if getGif
    mkdir( [saveroad,'/GIF' ,]);
    togetGif(washington,[saveroad,'/GIF/Clean_msi']);
end
if getGif;togetGif(noisy_msi,[saveroad,'/GIF/Noisy_msi']);end
if getImage
    mkdir( [saveroad,'/Image' ,]);
    imwrite(normalized(washington(:,:,band)),[saveroad,'/Image/Clean_msi.png']);
end
if getImage; imwrite(noisy_msi(:,:,band),[saveroad,'/Image/Noisy_msi.png']);end

i  = 1;
Re_msi{i} = noisy_msi;
[psnr(i), ssim(i), fsim(i), ergas(i),sam(i)] = MSIQA(washington * 255, Re_msi{i}  * 255);
enList = 1;
...
if EN_ENCAM_S
    ...
    Time(i) = toc;
    washingtons=Re_msi{i};
    save('washington_ENCAM','washingtons');
    [psnr(i), ssim(i), fsim(i), ergas(i),sam(i)] = MSIQA(washington * 255, Re_msi{i}  * 255);
...
subplot(numLine,5,1); imshow((washington(:,:,band)-minI)/(maxI-minI)),title( 'Clean');

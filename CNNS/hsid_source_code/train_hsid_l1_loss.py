...
def train_model_residual_lowlight_l1_gan():
    ...
    for epoch in range(start_epoch, num_epoch+1):
        ...
        for batch_idx, (noisy, cubic, label) in enumerate(train_loader):
            ...
            with torch.no_grad():
                fake = net(noisy, cubic)
            #print('noisy shape =', noisy.shape, fake_stage2.shape)
            #fake.detach()
            disc_fake_hat = disc(fake.detach()+noisy, noisy) # Detach generator
            ...
            residual = net(noisy, cubic)
            disc_fake_hat = disc(residual+noisy, noisy)
            gen_adv_loss = adv_criterion(disc_fake_hat, torch.ones_like(disc_fake_hat))  

            alpha = 0.2
            beta = 0.2          
            rec_loss = recon_criterion(residual, label-noisy)
        ...
        }, f"checkpoints/hsid_l1_gan_{epoch}.pth")
        ...
        if psnr > best_psnr:
            ...
            torch.save({
                ...
                'disc': disc.state_dict(),
                'disc_opt': disc_opt.state_dict()                  
            }, f"checkpoints/hsid_l1_gan_best.pth")
...
if __name__ == '__main__':
    ...
    #train_hsid_l1_loss()
    train_model_residual_lowlight_l1_gan()
...
def train_model_residual_lowlight_twostage_gan_l1_loss():
    ...
    for epoch in range(start_epoch, num_epoch+1):
        ...
        for batch_idx, (noisy, cubic, label) in enumerate(train_loader):
            ...
            l1_loss_weight = 0.8
            #下面这个loss分为两部分，第一部分用来恢复纹理，第二部分用来恢复亮度。
            rec_loss = l1_loss_weight * recon_criterion(residual, label-noisy) + \
                (1 - l1_loss_weight)  * loss_function_mse(residual_stage2, label-noisy)
        ...
        }, f"checkpoints/two_stage_hsid_dense_gan_l1_and_l2_loss{epoch}.pth")
        ...
        if psnr > best_psnr:
            ...
            torch.save({
                ...
                'disc': disc.state_dict(),
                'disc_opt': disc_opt.state_dict()                  
            }, f"checkpoints/two_stage_hsid_dense_gan_l1_and_l2_loss_best.pth")
...
if __name__ == '__main__':
    ...
    train_model_residual_lowlight_twostage_gan_l1_loss()
...
#non-Fixed noise distribution training
import glob
import numpy as np
...
class DenoisingDataset(Dataset):#数据加上噪声
    ...
    def __getitem__(self, index):
        ...
        #
        noise_x = torch.randn(batch_x.size()).mul_(np.random.randint(0, self.sigma) / 255.0)  # 加上高斯噪声，随机取强度

        for channels in range(30):
            batch_y[:, channels, :, :] = batch_y[:, channels, :, :] + torch.randn(batch_y[:, channels, :, :].squeeze().size()).mul_(np.random.randint(0, self.sigma) / 255.0)
        batch_x_noise = batch_x + noise_x

        return batch_x_noise,batch_y,batch_x
